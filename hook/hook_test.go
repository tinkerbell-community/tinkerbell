...
// mockReaderFrom implements io.ReaderFrom for testing TFTP handlers.
...
func TestTFTPHandler(t *testing.T) {
	...
	log := testr.New(t)
	h := config.TFTPHandler(log)

	if h == nil {
		t.Fatal("expected non-nil TFTP handler")
	}

	rf := &mockReaderFrom{}
	if err := h.ServeTFTP("vmlinuz-x86_64", rf); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if rf.buf.String() != content {
		t.Errorf("expected %q, got %q", content, rf.buf.String())
	}
	if rf.setSize != int64(len(content)) {
		t.Errorf("expected size %d, got %d", len(content), rf.setSize)
	}
}

func TestTFTPHandlerFileNotFound(t *testing.T) {
	dir := t.TempDir()
	config := NewConfig(WithImagePath(dir))
	log := testr.New(t)
	h := config.TFTPHandler(log)
	...
	rf := &mockReaderFrom{}
	err := h.ServeTFTP("nonexistent", rf)
	if err == nil {
		t.Fatal("expected error for missing file")
	}
	if !contains(err.Error(), "hook file not found") {
		t.Errorf("expected 'hook file not found' error, got: %v", err)
	}
}

func TestTFTPHandlerPathTraversal(t *testing.T) {
	dir := t.TempDir()
	config := NewConfig(WithImagePath(dir))
	log := testr.New(t)
	h := config.TFTPHandler(log)
	...
	rf := &mockReaderFrom{}
	err := h.ServeTFTP("../../etc/passwd", rf)
	if err == nil {
		t.Fatal("expected error for path traversal")
	}
}

func TestTFTPHandlerRPiPrefix(t *testing.T) {
	dir := t.TempDir()
	content := "rpi kernel"
	if err := os.WriteFile(filepath.Join(dir, "vmlinuz-aarch64"), []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{"serial prefix", "abcdef01/vmlinuz-aarch64"},
		{"mac prefix", "dc-a6-32-01-02-03/vmlinuz-aarch64"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rf := &mockReaderFrom{}
			if err := h.ServeTFTP(tt.filename, rf); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if rf.buf.String() != content {
				t.Errorf("expected %q, got %q", content, rf.buf.String())
...
func TestTFTPHandlerEmptyCacheDir(t *testing.T) {
	config := NewConfig(WithImagePath(""))
	log := testr.New(t)
	h := config.TFTPHandler(log)

	rf := &mockReaderFrom{}
	err := h.ServeTFTP("vmlinuz", rf)
	if err == nil {
		t.Fatal("expected error for empty cache dir")
	}
	if !contains(err.Error(), "cache directory not configured") {
		t.Errorf("expected 'cache directory not configured' error, got: %v", err)
	}
}
