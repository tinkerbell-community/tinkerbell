...
The network interface manager (`smee/internal/netif`) handles:
...
TINKERBELL_SMEE_NETIF_ENABLED=true

# Interface type: macvlan or ipvlan (default: macvlan)
TINKERBELL_SMEE_NETIF_INTERFACE_TYPE=macvlan

# Source interface (default: auto-detect from default gateway)
TINKERBELL_SMEE_NETIF_SRC_INTERFACE=eth0

# Enable leader election
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_ENABLED=true

# Leader election namespace (default: pod namespace)
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_NAMESPACE=tink-system

# Leader election lock name (default: smee-dhcp-proxy)
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_LOCK_NAME=smee-dhcp-proxy

# Leader election identity (default: pod hostname)
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_IDENTITY=smee-xyz

# Lease duration (default: 15s)
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_LEASE_DURATION=15s

# Renew deadline (default: 10s)
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_RENEW_DEADLINE=10s

# Retry period (default: 2s)
TINKERBELL_SMEE_NETIF_LEADER_ELECTION_RETRY_PERIOD=2s
```
...
```bash
tinkerbell \
  --smee-netif-enabled=true \
  --smee-netif-interface-type=macvlan \
  --smee-netif-src-interface=eth0 \
  --smee-netif-leader-election-enabled=true \
  --smee-netif-leader-election-namespace=tink-system
```
    ...
    spec:
      ...
      - name: smee
        ...
        - name: TINKERBELL_SMEE_DHCP_MODE
          value: "proxy"
        - name: TINKERBELL_SMEE_NETIF_ENABLED
          value: "true"
        - name: TINKERBELL_SMEE_NETIF_INTERFACE_TYPE
          value: "macvlan"
        - name: TINKERBELL_SMEE_NETIF_LEADER_ELECTION_ENABLED
          value: "true"
        - name: TINKERBELL_SMEE_NETIF_LEADER_ELECTION_NAMESPACE
          value: "tink-system"
---
apiVersion: v1
kind: ServiceAccount
...
kubectl get lease smee-dhcp-proxy -n tink-system -o yaml
