...
//
// AMT is the sparsest inventory source Tinkerbell supports: no drive WWN, no
// per-CPU serial, no firmware inventory beyond AMT's own version. The
...
func TestAttributesFromIntelAMTDevice(t *testing.T) {
	...
	device := &common.Device{
		...
		Drives: []*common.Drive{{
			Common: common.Common{
				Description: "MEDIA DEV 0",
				Model:       "Lexar SSD NM790 2TB",
				Serial:      "QH7495W100766P220J",
			},
			ID:            "MEDIA DEV 0",
	...
	// Model and serial come from the storage package AMT reports alongside each
	// media access device. Without them a drive is an anonymous capacity, and
	// two same-sized drives are indistinguishable once written to Hardware.
	if bd := attrs.BlockDevices[0]; bd.Model != "Lexar SSD NM790 2TB" || bd.SerialNumber != "QH7495W100766P220J" {
		t.Errorf("block device model/serial = %q/%q", bd.Model, bd.SerialNumber)
	}
	// AMT reports no WWN, so the field both collectors would join on stays
	// empty; serial is the only identity an in-band reading can be matched to.
	if got := attrs.BlockDevices[0].WWN; got != "" {
		t.Errorf("WWN should be empty for AMT, got %q", got)
	}
}
